# Video DownloadHelper

<div align="center">

🟡🔴🔵

### The easiest way to download videos from any website

[![Install on Chrome](https://img.shields.io/badge/Chrome-Install-4285F4?style=for-the-badge&logo=googlechrome&logoColor=white)](#)
[![Install on Firefox](https://img.shields.io/badge/Firefox-Install-FF7139?style=for-the-badge&logo=firefox&logoColor=white)](#)
[![Install on Edge](https://img.shields.io/badge/Edge-Install-0078D7?style=for-the-badge&logo=microsoftedge&logoColor=white)](#)

</div>

---

## 📖 Wiki Navigation

| Page | Description |
|------|-------------|
| [🏠 Home](Home) | Introduction & quick start |
| [⭐ Premium](Premium) | Lifetime license — no limits |
| [🆘 Support](Support) | Help, bug reports & FAQ |
| [💝 Donate](Donate) | Support development |
| [🎬 Features](Features) | Full feature list |
| [🔧 Installation](Installation) | Step-by-step setup |
| [❓ FAQ](FAQ) | Common questions |

---

## ⚡ What It Does

| Feature | Description |
|---------|-------------|
| 🎬 **HD Downloads** | Grab videos up to 4K at max speed |
| 📁 **Archive** | Save your favorite videos for offline viewing |
| 📡 **Live Streams** | Record HLS & DASH live streams |
| 🎵 **Audio** | Extract audio tracks from any video |
| 🌍 **55 Languages** | Localized for global users |

---

## 🚀 Quick Start

1. **Install** the extension from your browser's store
2. **Navigate** to a page with a video
3. **Click** the toolbar icon (it turns colored when media is detected)
4. **Download** — pick quality and hit the download button

---

## 📢 Links

- 🐛 [Issues & Bug Reports](../../issues)
- 💬 [Discussions](../../discussions)
- 📄 [License](../../blob/main/LICENSE)
